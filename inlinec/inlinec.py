...
self,"tfile",
___join(
tempfile.mkstemp(".cpp","temp",os.getcwd())
)),
___e(
self,"source",
getattr(self,"tfile")[1]
)),
___(
open(
getattr(self,"source"),"wb").write(c.encode("utf-8")),
___e(self,"sts",subprocess.check_output(f'cat {getattr(self,"source")} | g++ -x c++ - ',shell=True)),
__if(primt(subprocess.check_output("./a.out").decode('utf-8')),status==0,print(getattr(self,"sts")))
)),
