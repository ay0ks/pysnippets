...
__cf=type("__cf",(object,),{"__init__":(lambda self,c:__(None,___(___(___e(self,"tfile",___join(tempfile.mkstemp(".cpp","temp",os.getcwd()))),___e(self,"source",getattr(self,"tfile")[1])),___(___e(self,"f",open(getattr(self,"source"),"wb")),getattr(self,"f").write(c.encode("utf-8")),getattr(self,"f").close(),___e(self,"sts",subprocess.check_output(f'g++ -x c++ {getattr(self,"source")}',shell=True)),__if(print(subprocess.check_output("./a.out").decode('utf-8')),getattr(self,"sts")==0,print(getattr(self,"sts"))))),os.unlink(getattr(self,"source"),os.unlink("a.out"))))})
